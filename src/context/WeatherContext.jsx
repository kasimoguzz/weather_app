...
const WeatherProvider = ({children}) => {
    ...
    useEffect(() => {
        const getCityWeatherData = async (currentCity) => {
            setLoading(true)  
            try{
                const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${currentCity.name}&units=metric&appid=YOUR_APİ_KEY`).then( response => response.json() )
